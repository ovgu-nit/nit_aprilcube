...
# Custom file
ros2 launch nit_aprilcube aprilcube.launch.py file:=/path/to/my_config.yaml

# Override a single key from a profile
ros2 launch nit_aprilcube aprilcube.launch.py mode:=webcam override:="usb_cam: {parameters: {camera_ns: /custom}}"

# Standalone override (no base)
ros2 launch nit_aprilcube aprilcube.launch.py override:="apriltag: {parameters: {...}}"
```
...
```bash
ros2 launch nit_aprilcube aprilcube.launch.py mode:=sim
```
...
```bash
ros2 launch nit_aprilcube aprilcube.launch.py mode:=remote
```
...
Launch the deploy script with remote-configurations:
```bash
ros2 launch nit_aprilcube aprilcube.launch.py mode:=robot
```
- Rviz will open with panels showing the camera image, a view port with the TIAGo model and the planning scene objects (april cube and a pseudo box representing the table). No image annotation as it is an unecessary overhead.
- The head_follower node will follow the cube.
...
```bash
# Custom image topic
ros2 launch nit_aprilcube aprilcube.launch.py mode:=robot image_raw:=/my/camera/image

# Enable debug printer
ros2 launch nit_aprilcube aprilcube.launch.py mode:=sim use_tag_printer:=true

# Delay cube spawn (sim only)
ros2 launch nit_aprilcube aprilcube.launch.py mode:=sim spawn_delay_sec:=2.0
```
...
IncludeLaunchDescription(
    PythonLaunchDescriptionSource(
        os.path.join(nit_aprilcube_dir, 'launch', 'aprilcube.launch.py')
    ),
